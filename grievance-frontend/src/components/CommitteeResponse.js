...
function CommitteeResponse() {
  ...
  useEffect(() => {
    ...
    const loadGrievancesAndResponses = async () => {
      setLoading(true);
      try {
        const grievancesResponse = await axios.get(`https://react-student-grievance-portal-8.onrender.com/api/grievances?email=${userEmail}`);
        setGrievances(grievancesResponse.data);

        const responsesResponse = await axios.get(`https://react-student-grievance-portal-8.onrender.com/api/responses?email=${userEmail}`);
